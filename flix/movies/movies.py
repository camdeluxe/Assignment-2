...
def movies_by_search():
    ...
    if search_type == "title":
        movies, previous_title, next_title = services.get_movies_by_title(search_query, repo.repo_instance) # movies = list of dicts
...
def movies_by_genre():
    ...
    movies = services.get_movies_by_id(movie_ids[cursor:cursor + movies_per_page], repo.repo_instance) #list of movies as dicts
    ...
    # Construct urls for viewing article comments and adding comments.
    for movie in movies:
        movie['view_reviews_url'] = url_for('movies_bp.movies_by_genre', genre=genre_name, cursor=cursor, view_reviews_for=movie['id'])
        movie['add_review_url'] = url_for('movies_bp.review_movie', movie=movie['id'])
        movie['display_movie_url'] = url_for('movies_bp.display_movie', movie=movie['id'])
